            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {sales.map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{new Date(sale.created_at).toLocaleString('es-AR')}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{sale.seller_name}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{sale.items?.length || 0}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-primary-600">${sale.total.toFixed(2)}</td>
          ...
          <div className="bg-white rounded-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 font-medium"
              >
                <IterationCw />
            ...
            <div className="space-y-3">
              {refundState.items.map((it, idx) => (
                <div key={it.product_id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-md">
                  <div className="w-6">
                    <input type="checkbox" checked={it.selected} onChange={() => toggleSelectItem(idx)} />
                  </div>
                  <div className="flex-1">
